...
import { deleteLearningGoalAction } from '@/actions/roadmap';
import { Trash2, Loader2 } from 'lucide-react';
import { Dialog } from '@/components/ui/Dialog';
import { Button } from '@/components/ui/Button';

export function DeleteLearningGoalButton({
  goalId,
  goalTitle,
}: {
  goalId: string;
  ...
  const handleDelete = () => {
    startDelete(async () => {
      const res = await deleteLearningGoalAction(goalId);
      if (res && res.success) {
        setIsOpen(false);
      }
    });
      ...
      >
        <div className="space-y-4 font-mono text-xs">
          <p className="text-terminal-text-secondary">
            Are you sure you want to delete <strong className="text-terminal-text-primary">"{goalTitle}"</strong>?
          </p>
