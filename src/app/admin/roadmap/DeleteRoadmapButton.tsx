...
import { deleteRoadmapItemAction } from '@/actions/roadmap';
...
export function DeleteRoadmapButton({
  ...
  const handleDelete = () => {
    startDelete(async () => {
      const res = await deleteRoadmapItemAction(itemId);
      if (res && res.success) {
        setIsOpen(false);
      }
    });
  ...
  return (
    <>
      ...
      <Dialog
        ...
        title="Confirm Roadmap Item Deletion"
        maxWidth="sm"
      >
        <div className="space-y-4 font-mono text-xs">
          <p className="text-terminal-text-secondary">
            Are you sure you want to delete <strong className="text-terminal-text-primary">"{itemTitle}"</strong>?
          </p>
          <p className="text-[11px] text-terminal-text-muted">
            This item will be removed from the public roadmap board immediately.
          </p>
