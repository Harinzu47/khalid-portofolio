...
export function DeleteArticleButton({
  ...
  const handleDelete = () => {
    startDelete(async () => {
      const res = await deleteArticleAction(articleId, false);
      if (res && res.success) {
        setIsOpen(false);
      }
    });
      ...
      >
        <div className="space-y-4 font-mono text-xs">
          <p className="text-terminal-text-secondary">
            Are you sure you want to delete <strong className="text-terminal-text-primary">"{articleTitle}"</strong>?
          </p>
          <p className="text-[11px] text-terminal-text-muted">
            The article will be soft-deleted and removed from public listings immediately.
          </p>
