...
import { archiveADRAction } from '@/actions/adrs';
import { Archive, Loader2 } from 'lucide-react';
import { Dialog } from '@/components/ui/Dialog';
import { Button } from '@/components/ui/Button';

export function ArchiveADRButton({
  adrId,
  adrTitle,
}: {
  adrId: string;
  ...
  const handleArchive = () => {
    startArchive(async () => {
      const res = await archiveADRAction(adrId);
      if (res && res.success) {
        setIsOpen(false);
      }
    });
      ...
      >
        <div className="space-y-4 font-mono text-xs">
          <p className="text-terminal-text-secondary">
            Are you sure you want to archive <strong className="text-terminal-text-primary">"{adrTitle}"</strong>?
          </p>
          <p className="text-[11px] text-terminal-text-muted">
            The decision record will be soft-archived and removed from public listings immediately.
          </p>
...
export const DeleteADRButton = ArchiveADRButton;
